...
const WhatsAppFloat = () => {
  ...
  return (
    ...
    >
      {open && (
        <div className="bg-white border border-nanita-border rounded-2xl shadow-xl p-4 w-72 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <div className="flex items-start justify-between gap-2 mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-heading font-semibold text-nanita-ink leading-tight">
                  {store_name || 'Nanita'}
                </p>
                <p className="text-[11px] text-nanita-plum">Normalmente responde em minutos</p>
              </div>
            </div>
            <button
              onClick={() => setOpen(false)}
              className="text-nanita-plum hover:text-nanita-ink transition-colors"
              aria-label="Fechar"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          <p className="text-xs text-nanita-plum mb-3 leading-relaxed">
      ...
      {!open && showTeaser && (
        <button
          ...
          className="bg-white border border-nanita-border rounded-2xl rounded-br-sm shadow-lg px-3 py-2 max-w-[220px] text-left animate-in fade-in slide-in-from-right-2 duration-300 hover:border-[hsl(142_70%_45%)] transition-colors"
          aria-label="Abrir mensagem"
        >
          <p className="text-[11px] font-semibold text-[hsl(142_70%_38%)] mb-0.5">
            {store_name || 'Nanita'}
          </p>
          <p className="text-xs text-nanita-ink leading-snug">
      ...
      >
        ...
        {showBadge && !open && (
          <span
            className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1 rounded-full bg-nanita-jam text-white text-[11px] font-bold flex items-center justify-center shadow-md ring-2 ring-white animate-in zoom-in duration-300"

...
const OrderConfirmationPage = () => {
  ...
  if (isLoading) {
    return (
      <Shell>
        <p className="text-center text-nanita-plum">Carregando seu pedido...</p>
      </Shell>
    )
  }
    ...
    return (
      <Shell>
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <p className="max-w-md text-nanita-plum">
            {isError
    ...
    <Shell>
      <div className="flex flex-col gap-10">
        <div className="flex flex-col items-center gap-5 text-center">
          ...
          <div className="flex items-center justify-center rounded-full bg-nanita-sugar p-[22px]">
            <NanaMascot size={130} expression="wink" />
          </div>

          <div className="flex flex-col items-center gap-[10px]">
            <p className="nanita-eyebrow text-nanita-plum">
              PEDIDO {order.order_number} · {paidStamp(order.paid_at)}
            </p>
            <h1 className="font-heading text-[38px] font-semibold leading-[1.1] tracking-[-0.035em] text-nanita-ink md:text-[50px]">
            ...
            <p className="max-w-[480px] text-lg leading-[28px] text-nanita-plum">
              {paid
              ...
              <strong className="font-semibold text-nanita-ink">{order.customer_email}</strong>. Este
              pedido também fica guardado em Minha conta → Pedidos.
            </p>
          </div>

          <p className="flex flex-wrap items-baseline justify-center gap-2 text-[15px] text-nanita-plum">
            {paid ? 'Valor pago' : 'Valor do pedido'}
            <span className="font-heading text-xl font-semibold text-nanita-jam">

...
const SearchPage = () => {
  ...
  return (
    <div className="container max-w-3xl py-6 md:py-8">
      <h1 className="mb-4 font-heading text-2xl font-semibold text-nanita-ink md:mb-6 md:text-3xl">
        Busca
      </h1>

      <div className="relative mb-6 md:mb-8">
        <SearchIcon
          className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-nanita-jam"
          strokeWidth={2.2}
          aria-hidden
        />
        <input
          ...
          className="h-12 w-full rounded-pill border-2 border-nanita-rule bg-white pl-12 pr-12 text-base font-medium text-nanita-ink outline-none transition-colors placeholder:font-normal placeholder:text-nanita-plum focus:border-nanita-jam"
        />
        {query && (
          <button
            type="button"
          ...
          >
            <span className="flex h-6 w-6 items-center justify-center rounded-full bg-nanita-border">
              <X className="h-3.5 w-3.5 text-nanita-jam" strokeWidth={3} aria-hidden />
            </span>
          </button>
        )}
      </div>
      ...
      {searching && (
        <p className="mb-4 text-sm text-nanita-plum" aria-live="polite">
      ...
      {searching && results.length === 0 && (
        <div className="py-12 text-center">
          <p className="font-medium text-nanita-ink">Nenhum botton encontrado 😢</p>
          <p className="mx-auto mt-1 max-w-xs text-sm text-nanita-plum">
            Tenta o nome do anime, do grupo ou do personagem.
          </p>
          <Link
            to="/"
            className="mt-3 inline-block text-sm font-semibold text-nanita-jam hover:underline"
          >
            Explorar coleções
          </Link>
        </div>

...
const SlugReadonlyLine = ({ slug, derivedFromName, onEditInSeo }: Props) => (
  <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-muted/40 px-3 py-2.5">
    <Link2 className="h-3.5 w-3.5 shrink-0 text-estrelinha-admin-violet" aria-hidden="true" />
    ...
    <button
      type="button"
      onClick={onEditInSeo}
      className="shrink-0 text-xs font-semibold text-estrelinha-admin-violet hover:underline"
    >
      Editar em SEO →
    </button>
  </div>

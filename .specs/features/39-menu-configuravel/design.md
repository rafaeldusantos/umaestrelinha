...
| `renditionUrl` / `renditionSrcSet` | `@estrelinha/core/media` | **A condição mudou, e a dívida foi paga no merge com a `master` (2026-09-06).** Enquanto esta branch saiu de `feat/37-*`, o módulo era da `38` e não existia aqui — a arte saía em `<img loading="lazy">` no tamanho do original. Com as duas features na mesma árvore, os três banners passaram a pedir a foto **no tamanho da vaga**: **320px** na barra do computador, **104px** (quadrado) na folha do celular e **64px** na miniatura do painel — `src` no dobro (DPR 2), `srcset` com as duas larguras e `sizes` literal. Arte de fora do Storage volta **inalterada**, e cada vaga tem esse par coberto por teste |
